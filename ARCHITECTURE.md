...
│   │   ├── page.tsx                        # ホーム画面（HUD KPI・スキルグリッド・分析・3層称号エリア・実績バッジ導線）★ Phase9更新
│   │   ├── record/
...
│   │   ├── auth.ts                         # 認証ユーティリティ
│   │   ├── epithet.ts                      # 3層称号判定ロジック（二つ名・得意部位・レベル称号）★ Phase9全面刷新
│   │   ├── xpMultiplier.ts                 # 段位 → XP倍率変換
│   │   └── logger.ts                       # クライアントロガー
│   │
│   └── types/
│       └── index.ts                        # 全型定義・XP/レベル計算関数 ★ Phase9更新
│
...
#### `peer_evaluations`（他者評価ログ）★ Phase7更新
| 列 | カラム名 | 内容 |
|---|---|---|
...
#### `EpithetMaster`（二つ名マスタ）★ Phase9 列追加
| 列 | カラム名 | 内容 |
|---|---|---|
| A | ID | 識別ID（例: E001） |
| B | Category | 判定カテゴリ（現在は `styleCombo` 固定） |
| C | TriggerValue | 照合キー。`styleCombo` の場合は上位3 SubCategory を五十音順でカンマ結合した文字列（例: `基本,二段打ち,払い技`） |
| D | Name | 二つ名テキスト（例: `暴君`、`神速`）。UI表示時は自動でダブルクォーテーションで囲む |
| E | Rarity | ★ Phase9追加。レア度フラグ（`N` / `R` / `SR`）。UIの文字色判定に使用 |

> **Rarity の登録ルール（マスタ編集時）:**
> 上位3技（TriggerValue に含まれる SubCategory）の中に「抜き技」「摺り上げ技」「返し技」が何件あるかで決定する。
> - 2件以上: `SR`（Super Rare / 深紅 #8B2E2E + 太字 + 字間広）
> - 1件:     `R` （Rare / 藍鉄色 #2C4F7C）
> - 0件:     `N` （Normal / 墨黒 #2B2B2B）
>
> **設計方針（Phase9 データ駆動型）:**
> フロントエンドはこの列の値をそのまま UI の色・スタイルに反映する。
> コード側でレア度を計算しないため、マスタシートの編集だけで色・演出が変わり、再デプロイ不要。
> GAS は `getEpithetMasterData()` / `getDashboard()` でこの列も返すこと。
...
| `getEpithetMaster` | なし | 二つ名マスタ（★ Phase9: Rarity 列を含む） |
...
### ★ Phase9 更新: EpithetMasterEntry（Rarity 列追加）
...
export interface EpithetMasterEntry {
  ...
  description:  string;
  /**
   * ★ Phase9: レア度フラグ（EpithetMaster の Rarity 列）
   *   N  → Normal   （墨黒 #2B2B2B）
   *   R  → Rare     （藍鉄色 #2C4F7C）
   *   SR → Super Rare（深紅 #8B2E2E + font-bold + tracking-widest）
   * 旧マスタに存在しない場合があるため optional。
   */
  rarity?: 'N' | 'R' | 'SR';
}
```

### ★ Phase9 刷新: EpithetResult（3層構造）

```typescript
/**
 * calcEpithet() の戻り値。3層称号を保持する。
 *
 * Layer 1 - 二つ名:
 *   epithetName    … EpithetMaster の Name 列から取得（例: "暴君"）
 *   epithetRarity  … EpithetMaster の Rarity 列から取得（N / R / SR）
 *                    UIはこの値を直接参照して色・スタイルを決定する
 *
 * Layer 2 - 得意部位称号:
 *   favoritePartTitle … 部位名 + しきい値サフィックス（例: "小手一閃"）
 *                       部位: 面 / 小手 / 胴 / 突き
 *                       サフィックス: の神髄 / 免許皆伝 / 一閃 / の練達 / 修練者 / の嗜み
 *
 * Layer 3 - レベル称号:
 *   levelTitle … title_master から titleForLevel() で取得（例: "初段"）
 */
...
### ★ Phase9 更新: calcEpithet() シグネチャ

```typescript
// 変更前（Phase8まで）
calcEpithet(techniques: Technique[], epithetMaster: EpithetMasterEntry[]): EpithetResult

// 変更後（Phase9〜）
calcEpithet(
  techniques:    Technique[],
  epithetMaster: EpithetMasterEntry[],
  level:         number,          // ← 追加: levelTitle 算出に使用
  titleMaster?:  TitleMasterEntry[], // ← 追加: title_master 参照に使用
): EpithetResult

// 呼び出し側（page.tsx）
const epithet = calcEpithet(techniques, em, level, tm);
```
...
DashboardData: {
  ...
  epithetMaster?:   EpithetMasterEntry[]  // Phase9: rarity フィールドを含む
...
## 6.5 称号システム（Phase9 刷新）★ NEW

### 概要：3層構造

Phase9 にて称号システムを**3層構造**に刷新した。
各層は独立した判定ロジックを持ち、ホーム画面の称号エリアで垂直に表示される。

```
┌─────────────────────────────────────────────┐
│  [Lv.15] 初段                               │  ← Layer 3: レベル称号
│  "暴君"  吉木直人                            │  ← Layer 1: 二つ名（Rarity で色が変わる）
│  小手一閃  初段                              │  ← Layer 2: 得意部位称号 + リアル段位バッジ
└─────────────────────────────────────────────┘
```

### Layer 1: 二つ名（EpithetMaster / styleCombo）

**判定アルゴリズム（`calcEpithet()` 内）:**

```
1. user_techniques から subCategory ごとの累計ポイントを集計
2. 降順ソート（同点は localeCompare('ja') 五十音昇順）で上位3件抽出
3. 上位3件を五十音昇順でカンマ結合 → triggerKey
   例: ['払い技', '出端技', '基本'] → ソート後 → 'あ…か…は…' → 'か,は...'
4. EpithetMaster から category === 'styleCombo' かつ triggerValue === triggerKey を検索
5. マスタの Name・Rarity をそのまま返す（コードでレア度を計算しない）
6. 未登録: epithetName='未知なる', epithetRarity='N'
```

**データ駆動型レア度判定（Phase9 の設計の核心）:**

```
マスタシートの Rarity 列 ──→ EpithetMasterEntry.rarity ──→ UI の文字色
         ↑
  スプレッドシートを直接編集するだけで演出が変わる（再デプロイ不要）
```
...
- 対象部位: `面 / 小手 / 胴 / 突き`（technique_master の BodyPart 列と一致）
- 部位ごとに所属する全技の Points を合算し、最大部位を採用
- 同点の場合は BODY_PARTS 定義順（面→小手→胴→突き）で先勝ち

### Layer 3: レベル称号

`titleForLevel(level, titleMaster)` で `title_master` から取得。
既存ロジックを流用し、calcEpithet() 内部で levelTitle として返す。
...
### ✅ 称号システム刷新・3層構造（Phase9） ★ NEW

#### 設計方針: データ駆動型レア度判定
- 二つ名のレア度（N/R/SR）を**コードで計算せず、EpithetMaster の `Rarity` 列を直接参照**する設計に変更
- スプレッドシートの Rarity 列を編集するだけで色・スタイルが変わる（GAS/フロント 再デプロイ不要）

#### 3層構造の内訳
- **Layer 1 (二つ名):** user_techniques の SubCategory 累計ポイント上位3つを五十音順カンマ結合した TriggerKey で EpithetMaster を検索し、Name と Rarity を取得
- **Layer 2 (得意部位称号):** 面・小手・胴・突き の合計ポイント最大部位に 6 段階のしきい値サフィックスを付与（例: 小手一閃）
- **Layer 3 (レベル称号):** title_master から `titleForLevel()` で取得（既存ロジック流用）

#### ホーム画面の称号エリア刷新（`page.tsx`）
- 1行目: `[Lv.XX バッジ] [レベル称号]` — レベルバッジ（levelColor で着色）+ title_master の称号
- 2行目: `"二つ名" ユーザー名` — Rarity に応じた文字色（N:墨黒 / R:藍鉄色 / SR:深紅+太字+字間広）
- 3行目: `得意部位称号  リアル段位バッジ` — Layer 2 称号とリアル段位を横並び

#### 型・関数の変更
- `EpithetMasterEntry` に `rarity?: 'N' | 'R' | 'SR'` を追加
- `EpithetResult` を 4 フィールド構造に全面刷新（旧 `name` / `description` / `suffix` / `fullTitle` を廃止）
- `calcEpithet()` の引数に `level: number` と `titleMaster?: TitleMasterEntry[]` を追加
- GAS 側: `getEpithetMasterData()` / `getDashboard()` で Rarity 列もフロントに返すよう更新が必要
...
### Phase9 移行時（称号システム刷新）★ NEW

| 作業 | 内容 |
|---|---|
| スプレッドシート更新 | `EpithetMaster` シートに **E列 `Rarity`** を追加し、既存行に `N` / `R` / `SR` を入力する |
| GAS 更新 | `getEpithetMasterData()` と `getDashboard()` の EpithetMaster 返却部分に Rarity 列を含めるよう更新し、新バージョンとして再デプロイする |
| ファイル更新 | `src/lib/epithet.ts`・`src/app/page.tsx`・`src/types/index.ts` を上書き |
| 再デプロイ | Cloudflare Pages は GitHub push で自動デプロイ |

> **後方互換について:** EpithetMaster に Rarity 列がない（または空）既存行は、フロントエンドが `rarity` を `undefined` として受け取り、`'N'`（Normal）にフォールバックします。カラー表示は墨黒になりますが、エラーは発生しません。
...
- [ ] EpithetMaster の styleCombo 以外のカテゴリ拡張（`actionType`, `subCategory` 等の復活検討）
- [ ] 二つ名の SR 演出強化（アニメーション・グロー効果）
