...
import { fetchTechniques, updateTechniqueRating, fetchDashboard } from '@/lib/api';
...
export default function TechniquesPage() {
  ...
  const [epithetMaster, setEpithetMaster] = useState<EpithetMasterEntry[]>([]);
  const [userLevel, setUserLevel]         = useState(1);
  const [userTitleMaster, setUserTitleMaster] = useState<import('@/types').TitleMasterEntry[] | undefined>(undefined);
  ...
  useEffect(() => {
    // 技データとダッシュボードデータを並行取得
    Promise.all([fetchTechniques(), fetchDashboard()])
      .then(([techs, dash]) => {
        setTechniques(techs);
        setEpithetMaster(dash.epithetMaster ?? []);
        setUserLevel(calcLevelFromXp(dash.status.total_xp));
        setUserTitleMaster(dash.titleMaster);
      })
...
interface EpithetBannerProps {
  techniques:    Technique[];
  epithetMaster: EpithetMasterEntry[];
  level:         number;
  titleMaster?:  import('@/types').TitleMasterEntry[];
}

function EpithetBanner({ techniques, epithetMaster, level, titleMaster }: EpithetBannerProps) {
  const epithet    = calcEpithet(techniques, epithetMaster);
